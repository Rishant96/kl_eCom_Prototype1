    ...
    public class ProductsController : Controller
    {
        ...
        public ActionResult Index(int? vendorId)
        {
            if (vendorId is null || vendorId == 0)
                ...
                return RedirectToAction("Categories", new { vendorId });
            
            return View();
        }

        public ActionResult Categories(int? vendorId, int? catId)
        {
            if (vendorId is null || vendorId == 0)
                return View("Error");
            ...
            var store = db.Stores.FirstOrDefault(m => m.EcomUserId == vendor.Id);
            if (store is null) return View(store);
            ...
            if (catId == null)
            {
                model.Categories = db.Categories.Where(m => m.StoreId == store.Id && m.IsBase == true).ToList();
            }
            else
            {
        ...
        public ActionResult List(int? storeId, int? catId, int? vendorId, 
        ...
        public ActionResult List(ProductsListViewModel model, int? stockId, string returnUrl = "")
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Details(ProductsDetailsViewModel model)
        {
            ...
            return RedirectToAction("Details", new { id = model.StockId, returnUrl = model.ReturnUrl });
        }
        ...
        #region FiltersPartial
        public ActionResult FiltersPartial(ProductsListViewModel prodModel)
        {
        ...
        public ActionResult FiltersPartialPost(ProductsListViewModel model)
        {
            ...
            {
                storeId = (model.CategoryId == null) ?
                    null as int? :
                    db.Categories
                        .FirstOrDefault(m => m.Id == model.CategoryId)
                        .StoreId,
                catId = model.CategoryId,
                options.Price_MinValue,
                options.Price_MaxValue,
                options.Rating_Min,
        ...
        [HttpPost]
        public ActionResult FilterSort([Form] ShopFilteringOptions filteringOptions,
                                [Form] SelectedFilters selectedFilters)
        {
            int? storeID = TempData["storeId"] as int?;
            int? catID = TempData["catId"] as int?;
